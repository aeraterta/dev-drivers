...
int get_lsm303_temperature(void) {
    ...
    int ret = lsm303_register_to_int(ret_Val, 8);
    ret += 25;
...
void get_lsm303_accXYZ(float* pData, ACC_SCALE_CONFIG scale){
    ...
    for(uint8_t i = 0; i < 3; i++){
        ...
        pData[i] = round((lsm303_register_to_int(rawValue[i], 16) * sensitivity) / 10.0) / 100.0;
    }
}